...
function addRealTimeShipEditing(row) {
    ...
    document.getElementById('admin_shipinfo_semester').addEventListener('input', function() {
        if (admin_shipinfo_semesterCell) {
            if (this.value === 'second_semester') {
                admin_shipinfo_semesterCell.textContent = '2학기';  // 2학기로 표시
            } else if (this.value === 'first_semester') {
                admin_shipinfo_semesterCell.textContent = '1학기';  // 1학기로 표시
            }
        }
    });
    ...
    document.getElementById('admin_shipinfo_paymentType').addEventListener('input', function() {
        if (admin_shipinfo_paymentTypeCell) {
            if(this.value == 'PRE_PAYMENT'){
                admin_shipinfo_paymentTypeCell.textContent = '선지급';
            }else if(this.value == 'POST_PAYMENT'){
                admin_shipinfo_paymentTypeCell.textContent = '후지급';
            }
        }
    });
