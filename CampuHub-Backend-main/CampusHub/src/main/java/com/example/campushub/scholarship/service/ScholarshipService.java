...
// 'yyyy' 형식의 연도만 제공되므로, 월과 일을 추가하여 LocalDate로 변환
        String yearString = createDto.getYear();  // "2025"와 같은 연도 문자열
        LocalDate year = LocalDate.parse(yearString + "-01-01", DateTimeFormatter.ofPattern("yyyy-MM-dd"));

        SchoolYear schoolYear = schoolYearRepository.findBySemesterAndYear(
                        Semester.valueOf(createDto.getSemester()), year)
